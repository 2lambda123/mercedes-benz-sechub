...
public class DeveloperAdministration {
  ...
  private static final DeveloperProjectDetailInformation
      PROJECT_DETAIL_IMPORTER = new DeveloperProjectDetailInformation();
  ...
  public DeveloperAdministration(ConfigProvider provider,
    ...
    this.restHelper =
        createTestRestHelperWithErrorHandling(errorHandler, userContext);
  }

  public UIContext getUiContext() { return uiContext; }

  private TestRestHelper
  createTestRestHelperWithErrorHandling(ErrorHandler provider,
                                        UserContext user) {
    return createTestRestHelperWithErrorHandling(
        provider, user, RestHelperTarget.SECHUB_SERVER);
  }
  ...
  public void updateTestAPISuperAdmin(String userId, String apiToken) {
    IntegrationTestContext integrationTestContext =
        IntegrationTestContext.get();
    integrationTestContext.setSuperAdminUser(
        new FixedTestUser(userId, apiToken));
    integrationTestContext.rebuild();
  }

  private TestRestHelper
  createTestRestHelperWithErrorHandling(ErrorHandler provider, UserContext user,
                                        RestHelperTarget restHelperTarget) {
        ...
        return new DefaultResponseErrorHandler() {
          ...
          public void handleError(ClientHttpResponse response)
            ...
            try (BufferedReader br = new BufferedReader(
              ...
              provider.handleError("failed to read response body:" +
                                   e.getMessage());
            }
            ...
            errorOutput.append(
                "******************************************************************\n");
            errorOutput.append("Last URL call:")
                .append(TestRestHelper.getLastUrl());
            if (TestRestHelper.getLastData() != null) {
              errorOutput.append("\nWith data:")
                  .append(TestRestHelper.getLastData());
            }
            errorOutput.append("\n");
            errorOutput.append(
                "******************************************************************\n");
  ...
  public SecHubTestURLBuilder getUrlBuilder() {
    if (urlBuilder == null) {
      ...
      urlBuilder =
          new SecHubTestURLBuilder(provider.getProtocol(), port, server);
    }
    return urlBuilder;
  }
  ...
  public String fetchSignups() {
    return getRestHelper().getJSON(
        getUrlBuilder().buildAdminListsUserSignupsUrl());
  }

  public TestRestHelper getRestHelper() { return restHelper; }

  public PDSAdministration pds(String hostname, int port, String userid,
                               String apiToken) {
  ...
  public class PDSAdministration {
    ...
    public PDSTestURLBuilder getUrlBuilder() { return pdsUrlBuilder; }

    public PDSAdministration(String hostname, int port, String userid,
                             String apiToken) {
      pdsUrlBuilder = new PDSTestURLBuilder("https", port, hostname);
      TestUser user =
          new FixedTestUser(userid, apiToken, userid + "_pds@example.com");
      restHelper = new TestRestHelper(user, RestHelperTarget.SECHUB_PDS);
    }

    public String fetchServerConfigurationAsString() {
      return restHelper.getJSON(
          pdsUrlBuilder.buildAdminGetServerConfiguration());
    }

    public String getServerAlive() {
      return restHelper.headStringFromURL(
          pdsUrlBuilder.buildAnonymousCheckAlive());
    }

    public String createPDSJob(UUID sechubJobUUID, String productId,
                               Map<String, String> params) {
      return TestAPI.createPDSJobFor(sechubJobUUID, params, productId,
                                     restHelper, pdsUrlBuilder);
    }
    ...
    public String getJobResultOrError(String jobUUID) {
      return restHelper.getJSON(
          pdsUrlBuilder.buildGetJobResultOrErrorText(UUID.fromString(jobUUID)));
    }

    public String getJobStatus(String jobUUID) {
      return restHelper.getJSON(
          pdsUrlBuilder.buildGetJobStatus(UUID.fromString(jobUUID)));
    }

    public UUID getUUIDOfLastStartedJob() {
      return restHelper.getUUIDFromURL(
          pdsUrlBuilder.buildFetchLastStartedJobUUIDUrl());
    }
    ...
    public TestPDSServerConfiguration fetchServerConfiguration() {
      return JSONConverter.get().fromJSON(TestPDSServerConfiguration.class,
                                          fetchServerConfigurationAsString());
    }

    public String getJobOutputStream(UUID jobUUID) {
      return restHelper.getStringFromURL(
          pdsUrlBuilder.buildPdsUserFetchesJobOutputStreamUrl(jobUUID));
    }

    public String getJobErrorStream(UUID jobUUID) {
      return restHelper.getStringFromURL(
          pdsUrlBuilder.buildPDSUserFetchesJobErrorStreamUrl(jobUUID));
    }

    public String getJobMessages(UUID jobUUID) {
      return restHelper.getStringFromURL(
          pdsUrlBuilder.buildGetJobMessages(jobUUID));
    }

    public ProductIdentifier
    findProductIdentifier(TestPDSServerConfiguration config, String productId) {
    ...
    public String
    createExampleProperitesAsString(TestPDSServerConfiguration config,
                                    String productId) {
      ...
      for (TestPDSServerProductConfig c : config.products) {
        if (c.id.equals(productId)) {
          ...
          sb.append("# PDS server at :")
              .append(getUiContext().getPDSConfigurationUI().getHostName())
              .append(":")
              .append(getUiContext().getPDSConfigurationUI().getPort())
              .append("\n");
          sb.append(
              "# You can paste these parts directly into PDS server configuration!\n");
          sb.append("# ###################################\n");
          sb.append("\n");
          sb.append("# ---------------------\n");
          sb.append("# sechub call needs:   \n");
          sb.append("# ---------------------\n");
          sb.append(PDSDefaultParameterKeyConstants
                        .PARAM_KEY_PDS_CONFIG_PRODUCTIDENTIFIER +
                    "=" + c.id + "\n");
          sb.append("# Optional:\n");
          sb.append("# " +
                    SecHubProductExecutionPDSKeyProvider
                        .PDS_FORBIDS_TARGETTYPE_INTERNET.getKey()
                        .getId() +
                    "=true #does stop executing this one for internet\n");
          sb.append("# " +
                    SecHubProductExecutionPDSKeyProvider
                        .PDS_FORBIDS_TARGETTYPE_INTRANET.getKey()
                        .getId() +
                    "=true #does stop executing this one for intranet \n");
          sb.append(
              "# " +
              PDSProductExecutorKeyConstants.TIME_TO_WAIT_NEXT_CHECK_MILLIS +
              "=1000 #1000 is 1 second, 0 will fallback to 500 ms, if not set PDSInstallSetup values will be used\n");
          sb.append(
              "# " + PDSProductExecutorKeyConstants.TIME_OUT_IN_MINUTES +
              "=120 #would timeout pds connection after 2 hours, if not set PDSInstallSetup values will be used\n");
          sb.append(
              "# " + PDSProductExecutorKeyConstants.TRUST_ALL_CERTIFICATES +
              "=true # will trust any PDS server. Use this only in NON PRODUCTION environments (e.g when using self signed certificate)\n\n");
          sb.append("# ---------------------\n");
    ...
    public String fetchPDSAutoCleanupConfiguration() {
      TestAutoCleanupData configuration =
  ...
  public String acceptSignup(String userId) {
    getRestHelper().post(
        getUrlBuilder().buildAdminAcceptsUserSignUpUrl(userId));
    return "SENT";
  }

  public String declineSignup(String userId) {
    getRestHelper().delete(
        getUrlBuilder().buildAdminDeletesUserSignUpUrl(userId));
    return "SENT";
  }

  public String gGrantAdminRightsTo(String targetUser) {
    getRestHelper().post(
        getUrlBuilder().buildAdminGrantsSuperAdminRightsTo(targetUser));
    return "SENT";
  }

  public String revokeAddminRightsFrom(String targetUser) {
    getRestHelper().post(
        getUrlBuilder().buildAdminRevokesSuperAdminRightsFrom(targetUser));
    return "SENT";
  }
  ...
  public String fetchAdminList() {
    return getRestHelper().getStringFromURL(
        getUrlBuilder().buildAdminListsAdminsUrl());
  }

  public String fetchRunningJobsList() {
    return getRestHelper().getStringFromURL(
        getUrlBuilder().buildAdminFetchAllRunningJobsUrl());
  }
  ...
  public String createProject(String projectId, String description,
                              String owner, List<String> whiteListURLs,
    ...
    /* @formatter:on */
    return getRestHelper().postJson(
        getUrlBuilder().buildAdminCreatesProjectUrl(), json.toString());
  }
  ...
  public List<String> fetchProjectIdList() {
    ...
    SimpleTestStringList list =
        JSONConverter.get().fromJSON(SimpleTestStringList.class, json);
    return list;
  }
  ...
  fetchProjectDetailInformation(String projectId) {
    String json = fetchProjectInfo(projectId);
  ...
  public String fetchUserInfo(String userId) {
    return getRestHelper().getStringFromURL(
        getUrlBuilder().buildAdminShowsUserDetailsUrl(userId));
  }

  public String fetchUserInfoByEmailAddress(String emailAddress) {
    return getRestHelper().getStringFromURL(
        getUrlBuilder().buildAdminShowsUserDetailsForEmailAddressUrl(
            emailAddress));
  }

  public List<String> fetchProjectWhiteList(String projectId) {
    List<String> result = new ArrayList<>();
    String json = getRestHelper().getJSON(
        getUrlBuilder().buildAdminFetchProjectInfoUrl(projectId));
    TestJSONHelper jsonHelper = TestJSONHelper.get();
    JsonNode jsonNode = jsonHelper.readTree(json);
  ...
  public String fetchProjectScanLogs(String projectId) {
    String json = getRestHelper().getJSON(
        getUrlBuilder().buildAdminFetchesScanLogsForProject(projectId));
    return json;
  }

  public String fetchJSONReport(String projectId, UUID sechubJobUUID) {
    String json = getRestHelper().getJSON(
        getUrlBuilder().buildFetchReport(projectId, sechubJobUUID));
    return json;
  }

  public String fetchJobStatus(String projectId, String jobUUID) {
    return getRestHelper().getStringFromURL(
        getUrlBuilder().buildFetchJobStatus(projectId, jobUUID));
  }
  ...
  public void updateProjectWhiteList(String projectId, List<String> result) {
    ...
    getRestHelper().postJson(
        getUrlBuilder().buildUpdateProjectWhiteListUrl(projectId),
        sb.toString());
  }
  ...
  public void updateProjectMetaData(String projectId, String result) {
    ...
    getRestHelper().postJson(
        getUrlBuilder().buildUpdateProjectMetaData(projectId), sb.toString());
  }
  ...
  public String updateProjectDescription(String projectId, String description) {
    ...
    String url =
        getUrlBuilder().buildAdminChangesProjectDescriptionUrl(projectId);
  ...
  public String requestNewApiToken(String emailAddress) {
    getRestHelper().post(
        getUrlBuilder().buildAnonymousRequestNewApiToken(emailAddress));
    return "Sent request for new API token for email: " + emailAddress +
        " - New API token will be delivered to this address if user exists!";
  }

  public String enableSchedulerJobProcessing() {
    getRestHelper().post(
        getUrlBuilder().buildAdminEnablesSchedulerJobProcessing());
    return "triggered enable job processing";
  }

  public String disableSchedulerJobProcessing() {
    getRestHelper().post(
        getUrlBuilder().buildAdminDisablesSchedulerJobProcessing());
    return "triggered disable job processing";
  }

  public String refreshSchedulerStatus() {
    getRestHelper().post(
        getUrlBuilder().buildAdminTriggersRefreshOfSchedulerStatus());
    return "triggered refresh for scheduler status";
  }

  public String getStatusList() {
    return getRestHelper().getJSON(
        getUrlBuilder().buildAdminListsStatusEntries());
  }

  public String checkAlive() {
    return getRestHelper().headStringFromURL(
        getUrlBuilder().buildCheckIsAliveUrl());
  }

  public String checkVersion() {
    return getRestHelper().getStringFromURL(
        getUrlBuilder().buildGetServerVersionUrl());
  }

  public String triggerDownloadFullScan(UUID sechubJobUUID) {

    String url =
        getUrlBuilder().buildAdminDownloadsZipFileContainingFullScanDataFor(
            sechubJobUUID);
    return commonTriggerDownloadInBrowser(url);
  }

  /**
   * Creates temporary test user object and provides direct access to
   * integration test object: AsUser. So all things available in integration
   * tests can be done directly without additional methods
   *
  ...
  public WithSecHubClient withSecHubClientOnDefinedBinPath() {
    WithSecHubClient withSechubClient =
        createAsUserTestObject().withSecHubClient();
    String pathToBinaryparentFolder =
        ConfigurationSetup.SECHUB_PATH_TO_SECHUB_CLIENT_BINARY.getStringValue(
            null, false);
  ...
  public String updateGlobalMappings(String mappingId,
                                     String mappingDataAsJSON) {
  ...
  public String fetchProjectFalsePositiveConfiguration(String projectId) {
    return getRestHelper().getJSON(
        getUrlBuilder().buildUserFetchesFalsePositiveConfigurationOfProject(
            projectId));
  }

  public String markFalsePositivesForProjectByJobData(String projectId,
                                                      String json) {
    return getRestHelper().putJSON(
        getUrlBuilder().buildUserAddsFalsePositiveJobDataListForProject(
            projectId),
        json);
  }

  public void deleteFalsePositivesForProject(String projectId, UUID jobUUID,
                                             int findingId) {
    getRestHelper().delete(
        getUrlBuilder().buildUserRemovesFalsePositiveEntryFromProject(
            projectId, jobUUID.toString(), "" + findingId));
  }
  ...
  private String commonTriggerDownloadInBrowser(String url) {
    try {
      ...
      e.printStackTrace();
      return "Cannot open your system browser for url:" + url +
          ", please copy url and download in your browser manually.";
    }
  ...
  public void updateExecutionProfile(TestExecutionProfile updatedProfile) {
    asTestUser().updateProductExecutionProfile(updatedProfile.id,
                                               updatedProfile);
  }

  private AsUser asTestUser() { return as(userContext.toTestUser()); }
  ...
  public void removeProjectIdsFromProfile(String profileId,
                                          String... projectIds) {
  ...
  public void removeProjectIdsFromProfile(String profileId, List<String> list) {
    removeProjectIdsFromProfile(profileId,
                                list.toArray(new String[list.size()]));
  }

  public void changeProjectAccessLevel(String projectId,
                                       ProjectAccessLevel accessLevel) {
    asTestUser().changeProjectAccessLevel(new FixedTestProject(projectId),
                                          accessLevel);
  }

  public String fetchAutoCleanupConfiguration() {
    TestAutoCleanupData configuration =
        asTestUser().fetchAutoCleanupConfiguration();
    return TestJSONHelper.get().createJSON(configuration, true);
  }
  ...
  public String fetchProjectJobInfoForUser(String projectId, int pageSize,
                                           int page) {
    TestSecHubJobInfoForUserListPage listPage =
        asTestUser().fetchUserJobInfoList(new FixedTestProject(projectId),
                                          pageSize, page);
