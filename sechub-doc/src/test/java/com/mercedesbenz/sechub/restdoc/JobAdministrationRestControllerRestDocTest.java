...
@ContextConfiguration(classes = {JobAdministrationRestController.class,
                                 JobAdministrationRestControllerRestDocTest
                                     .SimpleTestConfiguration.class})
@WithMockUser(authorities = RoleConstants.ROLE_SUPERADMIN)
@ActiveProfiles({Profiles.TEST, Profiles.ADMIN_ACCESS})
@AutoConfigureRestDocs(uriScheme = "https",
                       uriHost = ExampleConstants.URI_SECHUB_SERVER,
                       uriPort = 443)
public class JobAdministrationRestControllerRestDocTest
    implements TestIsNecessaryForDocumentation {

  private static final int PORT_USED =
      TestPortProvider.DEFAULT_INSTANCE.getRestDocTestPort();

  @Autowired private MockMvc mockMvc;

  @MockBean JobInformationListService jobListService;

  @MockBean JobCancelService jobCancelService;

  @MockBean JobRestartRequestService jobRestartRequestService;

  @Before
  public void before() {
    List<JobInformationListEntry> list = new ArrayList<>();

    JobInformationListEntry info =
        new JobInformationListEntry(UUID.randomUUID(), LocalDateTime.now(),
                                    JobStatus.RUNNING, "project-id");
  ...
  public void restdoc_list_all_running_jobs() throws Exception {
    ...
    this.mockMvc
        .perform(get(apiEndpoint)
                     .contentType(MediaType.APPLICATION_JSON_VALUE)
                     .header(AuthenticationHelper.HEADER_NAME,
                             AuthenticationHelper.getHeaderValue()))
        ...
        .andDo(
            defineRestService()
                ...
                .document(
                    ...
                    responseFields(
                        fieldWithPath(
                            inArray(JobInformationListEntry.PROPERTY_JOB_UUID))
                            .description("The uuid of the running job"),
                        fieldWithPath(
                            inArray(JobInformationListEntry.PROPERTY_SINCE))
                            .description(
                                "Timestamp since when job has been started"),
                        fieldWithPath(
                            inArray(JobInformationListEntry.PROPERTY_STATUS))
                            .description("A status information "),
                        fieldWithPath(inArray(JobInformationListEntry
                                                  .PROPERTY_PROJECT_ID))
                            .description(
  ...
  public void restdoc_cancel_job() throws Exception {
    /* prepare */
    String apiEndpoint =
        https(PORT_USED).buildAdminCancelsJob(JOB_UUID.pathElement());
    ...
    this.mockMvc
        .perform(post(apiEndpoint, jobUUID)
                     .contentType(MediaType.APPLICATION_JSON_VALUE)
                     .header(AuthenticationHelper.HEADER_NAME,
                             AuthenticationHelper.getHeaderValue()))
        .andExpect(status().isOk())
        .andDo(
            defineRestService()
                ...
                .document(requestHeaders(
                          ...
                          pathParameters(parameterWithName(JOB_UUID.paramName())
  ...
  public void restdoc_restart_job() throws Exception {
    /* prepare */
    String apiEndpoint =
        https(PORT_USED).buildAdminRestartsJob(JOB_UUID.pathElement());
  ...
  public void restdoc_restart_job_hard() throws Exception {
    /* prepare */
    String apiEndpoint =
        https(PORT_USED).buildAdminRestartsJobHard(JOB_UUID.pathElement());
  ...
  private static String inArray(String field) { return "[]." + field; }
