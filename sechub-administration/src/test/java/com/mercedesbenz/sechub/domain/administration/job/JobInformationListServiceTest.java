...
class JobInformationListServiceTest {
  ...
  @Test
  void
  when_no_running_jobs_are_found_in_repository_an_empty_list_is_returned() {
    /* prepare */
  ...
  @Test
  void
  when_a_job_information_is_found_in_repository_an_enty_with_job_information_data_is_returned() {
  ...
  @Test
  void
  when_2_job_information_entries_found_in_repository_two_enty_with_job_information_data_is_returned() {
    ...
    information2.status =
        JobStatus.CREATED; // just a changed value - makes no sense here, but we
                           // use it as an example

    when(repository.findAllRunningJobs())
        .thenReturn(List.of(information1, information2));
  ...
  private void assertEntryContainsInfo(JobInformation information1,
                                       JobInformationListEntry entry1) {
