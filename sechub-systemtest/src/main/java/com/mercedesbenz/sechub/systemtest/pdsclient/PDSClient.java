...
public class PDSClient {
  ...
  public boolean isJobExisting(String pdsJobUUID) throws PDSClientException {
    try {
      ...
      throw new PDSClientException(
          "Cannot check if job " + pdsJobUUID +
  ...
  public PDSJobStatus fetchJobStatus(UUID pdsJobUUID)
      throws PDSClientException {
    try {
      ApiResponse<PDSJobStatus> status =
          fetchJobStatusApiResponse(pdsJobUUID.toString());
  ...
  private ApiResponse<PDSJobStatus> fetchJobStatusApiResponse(String pdsJobUUID)
      throws ApiException {
    ApiResponse<PDSJobStatus> status = commonApi.getAuthorized(
        "/api/job/" + pdsJobUUID + "/status", PDSJobStatus.class);
    return status;
  }

  public SecHubMessagesList fetchJobMessages(String pdsJobUUID)
      throws PDSClientException {
    try {
      ApiResponse<String> jsonResonse = commonApi.getAuthorized(
          "/api/job/" + pdsJobUUID + "/messages", String.class);
  ...
  public String fetchJobOutputStreamContentAsText(String pdsJobUUID)
      throws PDSClientException {
    try {
      return commonApi
          .getAuthorized("/api/job/" + pdsJobUUID + "/stream/output",
                         String.class)
          .getData();
    } catch (ApiException e) {
      throw new PDSClientException(
          "Cannot fetch output stream for PDS job:" + pdsJobUUID, e);
    }
  }

  public String fetchJobErrorStreamContentAsText(String pdsJobUUID)
      throws PDSClientException {
    try {
      return commonApi
          .getAuthorized("/api/job/" + pdsJobUUID + "/stream/error",
                         String.class)
          .getData();
    } catch (ApiException e) {
      throw new PDSClientException(
          "Cannot fetch error stream for PDS job:" + pdsJobUUID, e);
    }
  }

  public String fetchJobResultAsText(String pdsJobUUID)
      throws PDSClientException {
    try {
      return commonApi
          .getAuthorized("/api/job/" + pdsJobUUID + "/result", String.class)
          .getData();
    } catch (ApiException e) {
      throw new PDSClientException(
          "Cannot fetch result for PDS job:" + pdsJobUUID, e);
    }
  }

  public String fetchJobMetaDataAsText(String pdsJobUUID)
      throws PDSClientException {
    try {
      return commonApi
          .getAuthorized("/api/admin/job/" + pdsJobUUID + "/metadata",
                         String.class)
          .getData();
    } catch (ApiException e) {
      throw new PDSClientException(
          "Cannot fetch meta data for PDS job:" + pdsJobUUID, e);
    }
  }
}
