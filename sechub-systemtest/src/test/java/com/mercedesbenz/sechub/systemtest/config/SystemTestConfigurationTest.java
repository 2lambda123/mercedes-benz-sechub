...
class SystemTestConfigurationTest {
  ...
  private static final Logger LOG =
      LoggerFactory.getLogger(SystemTestConfigurationTest.class);

  private final static String DEFINED_PROJECT_NAME =
      "a-project-id-when-not-default-used";

  private static final String DEFINED_PROFILE =
      "a-defined-profile-when-not-default-used";

  private static final String DEFINED_REFERENCE_ID = "an-upload-reference-id";

  @Test
  void xray_license_config_can_be_read_and_contains_license_check()
      throws Exception {
    /* prepare */
    String path =
    ...
    /* execute */
    SystemTestConfiguration result =
  ...
  @Test
  void a_full_blown_setup_can_be_serialized_and_deserialized()
      throws Exception {

    /* prepare */
    SystemTestConfiguration configuration =
        createFullBlownExampleConfiguration();
    ...
    writeExample(configuration,
                 "gen_example_systemtest_full_blown_config.json");

    /* execute 2 */
    SystemTestConfiguration configLoaded =
        JSONConverter.get().fromJSON(SystemTestConfiguration.class, json);

    /* test 2 */
    assertNotNull(configLoaded);
    List<PDSSolutionDefinition> solutionsLoaded =
        configLoaded.getSetup().getLocal().get().getPdsSolutions();
  ...
  private SystemTestConfiguration createFullBlownExampleConfiguration() {
    ...
    configuration.getVariables().put("custom_data_txt",
                                     "${env.FROM_OUTSIDE}/subfolder1/data.txt");
  ...
  private void buildLocalSetup(SystemTestConfiguration configuration) {
    ...
    local.setComment(
        "This is a local setup - it does not use existing SecHub infrastructure, instead it does build up all by its own");
  ...
  private void configurePDSSolutions(LocalSetupDefinition local) {
    ...
    startStep1Script1.setPath(
        "./01-build-the-image.sh ${variables.myContainer.values}");
    ...
    stopStep1.setComment(
        "One single step to finally stop the gosec PDS solution when tests have been done.");
  ...
  private void configureSecHub(LocalSetupDefinition local) {
    ...
    secHubStartStep1StartStep1Script1.setPath(
    ...
    sechubConfig.setComment(
        "Here we can configure sechub. At least one executor entry is mandatory "
        + "(we must know which products shall be inside the profile). "
        +
    ...
    SecHubExecutorConfigDefinition executor1 =
        new SecHubExecutorConfigDefinition();

    Map<String, String> parameters1 = executor1.getParameters();
    parameters1.put("gosec.additional.parameter",
                    "${variables.custom_data_txt}");
    ...
    project1.setComment(
        "The profile can define a profile. When nothing is defined, the default will be '" +
        DefaultFallback.FALLBACK_PROFILE_ID.getValue() + "'");
  ...
  private void buildRemoteSetup(SystemTestConfiguration configuration) {
    ...
    remote.setComment(
        "This is a remote setup - we use an existing and configured SecHub cluster/instance");
  ...
  private void buildTest(SystemTestConfiguration configuration) {
    ...
    test1.setName("Test1");
    test1.setComment(
        "This test does a git checkout of a simple gosec application. After this we ensure the sechub result json file is as expected.");
    configuration.getTests().add(test1);

    ScriptDefinition testPrepareScript1 = new ScriptDefinition();
    testPrepareScript1.setComment(
        "The script call here would call the script inside the 'tests' subfoler of the solution");
    testPrepareScript1.setPath(
        "./../tests/checkout-simple-go-project-withouth-sechub-json.sh ${runtime." +
    ...
    testPrepareScript2.setComment(
        "Just another script call as an example for possibility of multiple scripts + an environment variable from 'outside'...");
    ...
    test1execute1.setComment(
        "This part can be defined only once. It describes what is executed");
    RunSecHubJobDefinition runSecHubJob1 = new RunSecHubJobDefinition();
    runSecHubJob1.setComment(
        "If a project is defined, it will be used, otherwise always '" +
        DefaultFallback.FALLBACK_PROJECT_NAME.getValue() +
        "' is used as default");
    ...
    upload.setComment(
        ...
        DefaultFallback.FALLBACK_UPLOAD_REF_ID.getValue() +
        " is used as default");
    ...
    secretScan.getNamesOfUsedDataConfigurationObjects().add(
    ...
    AssertSechubResultDefinition assertSecHubResult1 =
        new AssertSechubResultDefinition();
    test1Assert1.setSechubResult(Optional.of(assertSecHubResult1));

    AssertEqualsFileDefinition test1Assert1equalsFile1 =
        new AssertEqualsFileDefinition();
    ...
    AssertContainsStringsDefinition test1Assert1ContainsStrings =
        new AssertContainsStringsDefinition();
    test1Assert1ContainsStrings.setComment(
        "Checks if the sechub result file contains the given strings.");
    test1Assert1ContainsStrings.setValues(
        Arrays.asList("CWE-89", "SQL-Injection", "Improper"));
    assertSecHubResult1.setContainsStrings(
        Optional.of(test1Assert1ContainsStrings));
