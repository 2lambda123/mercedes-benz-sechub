 ...
 *
 * Because AE algorithms do both, encrypt and hash at the same time they are
 * difficult to incorrectly use.
 *
 ...
 * single data entry point generates a nonce and looks up into the list of used
 ...
 * unique and has never been used before. The consequence is, that data cannot
 * be written in parallel, which is unacceptable and in the case of a
 * distributed application like SecHub impossible as it would slow down the rate
 * at which date can be inserted into the database.
 *
 ...
 *
 * This package also contains classes which can help with algorithm rotation and
 * key rotation:
 *
