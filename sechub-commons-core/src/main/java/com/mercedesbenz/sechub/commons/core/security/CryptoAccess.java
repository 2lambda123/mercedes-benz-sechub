...
public class CryptoAccess<T extends Serializable> {
   ...
   */
  public static final CryptoAccess<char[]> CRYPTO_CHAR_ARRAY =
      new CryptoAccess<>();

  public CryptoAccess() { secretKey = getkeyGen(transformation).generateKey(); }
  ...
  private static KeyGenerator getkeyGen(char[] transformation) {
    ...
    try {
      KeyGenerator keyGen =
          KeyGenerator.getInstance(new String(transformation));
      keyGen.init(128);
  ...
  public SealedObject seal(T object) {
    try {
      ...
      throw new IllegalStateException(
          "cannot create sealed object for given objects", e);
    }
  }
